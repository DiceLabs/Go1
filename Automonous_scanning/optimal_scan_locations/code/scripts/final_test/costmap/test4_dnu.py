...
if __name__ == "__main__":
    ...
    # Calculate the centroids of scan locations and visible corners for each scan position
    for scan_location, visible_corners in visible_corners_dict.items():
        scan_location_centroid = calculate_centroid([scan_location])
        visible_corners_centroid = calculate_centroid(visible_corners)
        update_scan_position = calculate_centroid([scan_location] + visible_corners)

        # Draw the scan location centroid in red
        cv2.drawMarker(occupancy_grid_image, scan_location_centroid, (0, 0, 255), cv2.MARKER_STAR, markerSize=20)

        # Draw the visible corners centroid in red
        cv2.drawMarker(occupancy_grid_image, visible_corners_centroid, (0, 0, 255), cv2.MARKER_CROSS, markerSize=15)

        # Draw the updated scan position centroid in red
        cv2.drawMarker(occupancy_grid_image, update_scan_position, (0, 0, 255), cv2.MARKER_DIAMOND, markerSize=25)

    # Show the occupancy grid image using cv2.imshow
    cv2.imshow("Optimal Scanning positions with Centroids", occupancy_grid_image)
