...
def is_visible(scan_location, corner, occupancy_grid_map, min_distance=15):
    scan_x, scan_y = scan_location
    corner_x, corner_y = corner

    # Calculate the distance between the scan_location and corner point
    distance = np.sqrt((corner_x - scan_x) ** 2 + (corner_y - scan_y) ** 2)
...
def find_scan_locations(corners, occupancy_grid_map, min_distance=15):
    scan_locations = []
    for corner in corners:
        visible_from_any_scan_location = False
        for scan_location in scan_locations:
            if is_visible(scan_location, corner, occupancy_grid_map, min_distance):
...
if __name__ == "__main__":
    ...
    scan_locations = find_scan_locations(corners, reduced_occupancy_grid_map, min_distance=15)
